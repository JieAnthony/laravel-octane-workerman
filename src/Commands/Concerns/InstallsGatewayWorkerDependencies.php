...
use GatewayWorker\BusinessWorker as Worker;

trait InstallsGatewayWorkerDependencies
{
    /**
    ...
    protected function ensureWorkermanPackageIsInstalled()
    {
        ...
        if (!$this->confirm('laravel-octane-workerman requires "workerman/gateway-worker" and "workerman/gatewayclient". Do you wish to install it as a dependency?')) {
            $this->error('laravel-octane-workerman requires "workerman/gateway-worker" and "workerman/gatewayclient"');

            return false;
        }

        $command = $this->findComposer() . ' require workerman/gateway-worker:^3.0 workerman/gatewayclient:^3.0 --with-all-dependencies';
